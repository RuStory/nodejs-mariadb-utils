...
var startsWith = function(value, str) {
	return value.slice(0, str.length) === str; //якшо початком value є str, тоді харашо
};

/**
 * for checking in, not in conditions
 * @param value
 * @param str
 * @returns {boolean}
 */
var endsWith = function(value, str) {
	return value[value.length-1] === str; //якшо початком value є str, тоді харашо
};
...
module.exports = {
	...
	upgrade: function(connection) {
		...
		/**
		 * added not in possibility
		 * checked second )
		 * corrected checking '?' symbol(only if in the end)
		 * @param where
		 * @returns {string}
		 */
		//done
		...
		/**
		 * added checking if table exists
		 * @param table
		 * @param fields
		 * @param where
		 * @param order
		...
		/**
		 * added checking columns for existing
		 * @param table
		 * @param row
		 * @param callback
		 */
		...
		//done
		/**
		 * forms where before update is calling
		 * @param table
		 * @param row
		 * @param callback
		 */
		...
		//done
		/**
		 * added possibility execute without where
		 * @param table
		 * @param where
		 * @param callback
		 * @returns {*}
		...
		//my
		/**
		 * creates sql which gives all columns using join
		 * driver works incorrect
		 * @param tables
		 * @param on
		 * @param where
		 * @param callback
